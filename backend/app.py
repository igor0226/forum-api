...
from perf.monitoring.worker import perf_monitoring_worker
from perf.monitoring.queue import perf_monitoring_queue
...
if use_perf_logger:
    performance_logging_worker = Process(
        ...
        args=(perf_logger_queue,),
    )
    performance_logging_worker.daemon = True
    performance_logging_worker.start()

    performance_monitoring_worker = Process(
        target=perf_monitoring_worker,
        args=(perf_monitoring_queue,),
    )
    performance_monitoring_worker.daemon = True
    performance_monitoring_worker.start()
